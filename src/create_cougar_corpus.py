...
def get_data(cha_folder: Path) -> dict:
    ...
    converted_folder = (cha_folder / "an1/converted")
    csvs = list(converted_folder.glob("*.csv"))
    for csv in tqdm(csvs):
        print(type(csv))
        wav_file = '_'. join(str(csv).split('/')[-1].split('_')[:4])
        print(wav_file)
        ...
        data = {"age" : age, "raw_age": cha_file.stem, "data": defaultdict(list), "filename": wav_file}
